...
""")
        ...
        app = apps[0]
        assert app.build_status == BuildStatus.DISABLED
        assert 'Not enabled by manifest rules for target esp32c3' == app.build_comment
    ...
    def test_target_not_in_default_build_targets(self, tmp_path):
        ...
        app = apps[0]
        assert app.build_status == BuildStatus.DISABLED
        assert f'Project {tmp_path / "test_app"} only supports targets esp32,esp32s2' == app.build_comment
...
""")
        ...
        app.check_should_test()
        assert (
            'Disabled by manifest test rule: IDF_TARGET == "esp32c3" (reason: Testing not stable on this target)'
            == app.test_comment
        )
...
""")
        ...
        app.check_should_test()
        assert 'Not enabled for testing on target esp32s3 by manifest rules' == app.test_comment
...
""")
        ...
        assert (
            'Disabled by manifest test rule: IDF_TARGET == "esp32c3" (temporary) (reason: Flaky tests on this hardware)'
